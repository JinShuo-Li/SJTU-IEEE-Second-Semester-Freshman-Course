...
def assert_close(a, b, tol=1e-7):
	if isinstance(a, Fraction) or isinstance(b, Fraction):
		...
		b = float(b)
	if abs(a - b) > tol:
		raise AssertionError(f"Expected {a} ~= {b} (tol={tol})")


def assert_matrix_close(a, b, tol=1e-7):
	if a.dim != b.dim:
		raise AssertionError(f"Shape mismatch: {a.dim} vs {b.dim}")
	for i in range(a.dim[0]):
		for j in range(a.dim[1]):
			assert_close(a.data[i][j], b.data[i][j], tol)


def as_float(matrix):
	return Matrix(matrix.to_float_data(), tolerance=matrix.tolerance, dtype="float")


def assert_orthonormal(q, tol=1e-7):
	qtq = q.transpose() * q
	identity = Matrix.identity(qtq.dim[0], tolerance=tol, dtype="float")
	assert_matrix_close(as_float(qtq), identity, tol)


def _print_test_details(test_name, details):
	if not details:
		return
...
def test_basic_ops():
	a = Matrix([[1, 2], [3, 4]])
	if a.shape() != (2, 2):
		raise AssertionError("shape() failed")
	if a[0, 1] != Fraction(2):
		raise AssertionError("__getitem__ failed")

	b = a.transpose()
	assert_matrix_close(b, Matrix([[1, 3], [2, 4]]))

	c = a + Matrix([[5, 6], [7, 8]])
	assert_matrix_close(c, Matrix([[6, 8], [10, 12]]))

	d = a - Matrix([[1, 1], [1, 1]])
	assert_matrix_close(d, Matrix([[0, 1], [2, 3]]))

	e = 3 * a
	assert_matrix_close(e, Matrix([[3, 6], [9, 12]]))

	f = a ** 2
	assert_matrix_close(f, Matrix([[7, 10], [15, 22]]))

	g = a.reshape((1, 4))
	assert_matrix_close(g, Matrix([[1, 2, 3, 4]]))

	all_sum = a.sum()
	assert_matrix_close(all_sum, Matrix([[10]]))

	col_sum = a.sum(axis=0)
	assert_matrix_close(col_sum, Matrix([[4, 6]]))

	row_sum = a.sum(axis=1)
	assert_matrix_close(row_sum, Matrix([[3], [7]]))

	rank_a = a.rank()
	if rank_a != 2:
		raise AssertionError("rank() failed")

	h = Matrix([[1, 2], [2, 4]])
	rank_h = h.rank()
	if rank_h != 1:
		raise AssertionError("rank() failed for dependent rows")
...
def test_determinant_inverse():
	...
	det = a.determinant()
	if det != Fraction(10):
		raise AssertionError("determinant() failed")

	inv = a.inverse()
	identity = I(2)
	assert_matrix_close(a * inv, identity)
...
def test_lu_decomposition():
	a = Matrix([[2, 3, 1], [4, 7, 7], [2, 5, 2]])
	p, l, u = a.lu_decomposition(pivoting=True)
	assert_matrix_close(l * u, p * a)
...
def test_cholesky():
	a = Matrix([[4, 2], [2, 3]], dtype="float")
	l, lt = a.cholesky_decomposition()
	recon = l * lt
	assert_matrix_close(recon, a, tol=1e-6)
...
def test_gram_schmidt():
	a = Matrix([[1, 1], [1, 0], [1, -1]], dtype="float")
	q = a.gram_schmidt(method="mgs")
	assert_orthonormal(q, tol=1e-6)
...
def test_qr_decomposition():
	a = Matrix([[1, -4], [2, 3], [2, 2]], dtype="float")
	q, r = a.qr_decomposition()
	assert_orthonormal(q, tol=1e-6)
	assert_matrix_close(q * r, a, tol=1e-6)
...
def test_eigen_jacobi():
	a = Matrix([[2, 1], [1, 2]], dtype="float")
	vals, vecs = a.eigenpairs_jacobi(max_iterations=2000, sort=True)
	if len(vals) != 2:
		raise AssertionError("eigenpairs_jacobi() failed")
	assert_close(vals[0], 3.0, tol=1e-6)
	assert_close(vals[1], 1.0, tol=1e-6)
	assert_orthonormal(vecs, tol=1e-6)
...
def test_eigen_qr():
	a = Matrix([[5, 4], [1, 2]], dtype="float")
	vals = a.eigenvalues_qr(iterations=200, qr_method="householder")
	vals = sorted(vals, reverse=True)
	assert_close(vals[0], 6.0, tol=1e-5)
	assert_close(vals[1], 1.0, tol=1e-5)
...
def test_svd():
	a = Matrix([[3, 1, 1], [-1, 3, 1]], dtype="float")
	u, sigma, vt = a.singular_value_decomposition()
	recon = u * sigma * vt
	assert_matrix_close(recon, a, tol=1e-5)
	assert_orthonormal(u, tol=1e-6)
	assert_orthonormal(vt.transpose(), tol=1e-6)
...
def test_jordan_normal_form():
	a = Matrix([[2, 1], [0, 2]])
	j = a.jordan_normal_form()
	assert_matrix_close(j, Matrix([[2, 1], [0, 2]]))
...
def test_kronecker_and_concat():
	...
	row_cat = concatenate([a, b], axis=0)
	assert_matrix_close(row_cat, Matrix([[1, 2], [3, 4], [0, 5], [6, 7]]))

	col_cat = concatenate([a, b], axis=1)
	assert_matrix_close(col_cat, Matrix([[1, 2, 0, 5], [3, 4, 6, 7]]))
...
def main():
	...
	for test in tests:
		try:
			...
			_print_test_details(test.__name__, details)
